                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mt-4">
                        ...
                        <p class="text-gray-600 dark:text-gray-400 mt-2">
                            <strong>{{ __('Joined at:') }}</strong> {{ \Carbon\Carbon::parse($data['client']->created_at)->format('d/m/Y') }}
                        </p>
                    </div>

                    <div class="mt-6">
                        <a href="{{ route('client.edit', $data['client']->id) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded">
                            {{ __('Edit Client') }}
                        </a>
                        <form action="{{ route('client.destroy', $data['client']->id) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
                        ...
                        <ul class="mt-4">
                            @foreach ($data['exercises'] as $index => $exercise)
                                <li class="mb-4 p-4 bg-gray-100 dark:bg-gray-700 rounded-md shadow-sm">
                                    ...
                                    <p class="mt-2 text-gray-600 dark:text-gray-400">
                                        <strong>{{ __('Created at:') }}</strong> {{ \Carbon\Carbon::parse($exercise->created_at)->format('d/m/Y') }}
                                    </p>
                                    @php
                                    ...
                                    @if($review && $review->done)
                                        <p class="mt-2 text-green-500">
                                            {{ __('Completed at:') }} {{ \Carbon\Carbon::parse($review->done_at)->format('d/m/Y') }}
                                        </p>
                                    @else
